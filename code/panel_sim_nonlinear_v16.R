...
######################################################################################################################
  ...
  simulate_panel_data <- function(units, periods, T0, num_covariates,sigma,SNR) {
    ...
    #### DGP Settings: Nonlinear ######
    data<-data[order(data$Unit,data$Period),]
    data$Eta <- rep(rnorm(units),each=periods )

    data$Mean <- 0.25*(data$Covariate_2)^2 + 1.5*(data$Covariate_4) + data$Eta   ##Signal without treatment
...
sim<-function(){
  ...
  ##path at laptop
  ##path <-"C:/Users/ezhang2/OneDrive - Seattle University/2024 CSE Summer UG Research Award/R code/sim_nonlinear16/"

  ##path at desktop
  path <-"C:/Users/yongl/OneDrive - Seattle University/2024 CSE Summer UG Research Award/R code/sim_nonlinear16/"
